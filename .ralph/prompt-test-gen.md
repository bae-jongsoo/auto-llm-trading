...
3. 아래 태스크 스펙을 읽으세요.
4. 이 스펙에 맞는 pytest 통합 테스트를 작성하세요.

테스트 구조:
- 통합 테스트(tests/)에서는 handlers.py 함수를 호출하여 테스트하세요.
  services.py 함수를 테스트 대상으로 직접 검증하지 마세요.
- handlers.py는 여러 서비스 함수를 조합하여 비즈니스 흐름을 완성하는 진입점입니다.
  서비스 함수의 단순 re-export가 아닙니다.
- 서비스 함수의 엣지 케이스는 구현 단계에서 apps/앱이름/tests/에 단위 테스트로 작성합니다.
...
규칙:
- 통합 테스트는 프로젝트 루트의 tests/ 폴더에 작성하세요.
...
- 완료 전 자가 검증을 수행하세요:
  ...
  3. 이 단계는 테스트 코드만 작성하는 단계입니다. 핸들러/서비스/외부 모듈의 실제 구현은 2차 루프에서 합니다.
