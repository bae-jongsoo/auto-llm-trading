...
def test_KIS_현재정보조회_성공():
    ...
    with patch("shared.external.kis_quote.requests.post") as mock_post:
        with patch("shared.external.kis_quote.requests.get") as mock_get:
            mock_post.return_value = _http_응답({"access_token": "token-abc"})
            mock_get.return_value = _http_응답({"rt_cd": "0", "output": output})

            result = fetch_inquire_price("005930")
...
def test_현재정보_normalize_지원하지_않는_종목코드_실패():
    with pytest.raises(ValueError, match="지원하지 않는 종목코드"):
        normalize_market_snapshot("999999", _현재정보_payload())
...
def test_현재정보_수집_성공_stock_codes_미지정시_대상10종목_전체수집():
    ...
    with patch("apps.market.services.fetch_inquire_price", side_effect=_side_effect) as mock_fetch:
...
@pytest.mark.django_db
def test_현재정보_수집_지원하지_않는_종목코드_실패():
    with pytest.raises(ValueError, match="지원하지 않는 종목코드"):
        collect_market_snapshots(stock_codes=["999999"])
